...
def has_suspicious_unicode_mix(text: str) -> bool:
    """
    Слабая эвристика: если в строке есть латиница и одновременно встречаются
    часто-подменяемые кириллические символы (а, е, о, р, с, у, б, х).
    """
...
class UrlScanClient:
    ...
    def submit_scan(self, url: str) -> Tuple[int, Optional[str], Optional[str]]:
        """
        Возвращает: (http_status, uuid, error_text)
        """
...
class PhishGuard:
    """
    Подсчёт риска:
    - Для текста: накапливаем "баллы риска" (0..100), но контролируем вклад по категориям.
    ...
    def analyze_url(self, raw_url: str) -> UrlAnalysis:
        ...
        if "%" in full or re.search(r"(javascript:|eval\()", full, flags=re.I):
